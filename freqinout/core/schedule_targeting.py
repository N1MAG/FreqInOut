...
def schedule_target_identity_parts(row: Mapping[str, Any]) -> Tuple[str, str, str]:
    scope, device_profile_id, operating_profile_id = normalize_schedule_target(
        row.get("target_scope"),
        target_device_profile_id=row.get("target_device_profile_id"),
        target_operating_profile_id=row.get("target_operating_profile_id"),
    ...
    return (
        scope,
        str(int(device_profile_id)) if device_profile_id is not None else "",
        str(int(operating_profile_id)) if operating_profile_id is not None else "",
    )
