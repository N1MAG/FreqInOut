...
- The old `All Radios` drawer is redundant in multi-radio command mode and should remain hidden there. Radio activation and detailed configuration remain manageable from Settings. The control bar displays active controllable radios only; configured-but-inactive radios remain visible in Settings and Station Health but must not appear as commandable operating lanes.
...
- Activating a radio through `Use Radio` must persist `runtime_active=1` and refresh the same runtime projection used by the control bar, scheduler, Station Health, and Settings status surfaces. Selecting or focusing a radio card in the control bar is an in-memory UI target change only; it must not mutate `runtime_active`, `runtime_primary`, endpoint fields, assigned plans, or legacy compatibility settings.
...
- Legacy flat settings may be projected from a single active/compatibility radio for transitional single-radio surfaces, but must not be mirrored back into `device_profiles` endpoint fields while more than one radio is active. Per-radio endpoints are authoritative once multi-active operation is enabled; saving one profile must preserve sibling profiles' active state and endpoints unless the user explicitly edits those sibling profiles.
...
- Selecting a control-bar card is an in-memory command focus only. It must not toggle runtime active state, change `runtime_primary`, project legacy settings, rewrite endpoint rows, or change assigned plans.
...
Guidance model:
- The `Setup Steps` cards are the primary explanation surface. Radio, Software, Connection, RF Guard, Schedule, and Review each show one compact status and one useful detail.
- The `Setup Steps` cards live in the wizard shell above the step content so they remain visible as the user moves through Radio, Software, Connection, RF Guard, Schedule, and Review. They must not be nested inside the Software form.
...
- `Configure Automatically` review copy is generated by a core guided-setup helper and must remain bounded in the visible wizard surface: show filled fields, preserved fields, base-folder use, and only a few detection notes; keep full detail available through tooltip/log/debug surfaces instead of expanding the normal walkthrough.
- After `Configure Automatically` runs, the wizard advances to Connection only when endpoint/path fields apply. If the selected setup type has no FIO-controlled connection fields, the wizard skips Connection and advances to RF Guard.
...
Wizard shell:
- The Add Radio dialog exposes the setup as six steps: Radio, Software, Connection, RF Guard, Schedule, and Review.
...
- RF Guard is reviewed before Schedule assignment so supported bands, shared antenna paths, transmit/PTT domains, and front-end guard groups can warn before the operator chooses or assigns a Frequency Plan.
- The RF Guard step presents supported bands first, then any shared RF path/group fields. PTT and antenna/front-end guard details belong in this safety step, not as unrelated optional metadata.
- Selecting the RF Guard wizard step must render the actual RF Guard controls: supported bands, unsupported-band action, shared antenna path, shared transmit/PTT path, same-band guard, close-frequency guard, front-end/amplifier guard groups, and notes as applicable. The RF Guard page must not show only a wrapper or heading.
- The Review step uses structured cards instead of a prose wall. At minimum it separates Radio Profile, Software and Control, Endpoints, RF Guard and Schedule, and Files/Launch/App Configuration. Each card uses concise operator language and only expands technical details through a deliberate details affordance or tooltip.
...
- The Review step must highlight endpoint and app configuration conflicts against existing saved radio profiles before Save. Duplicate FIO-controlled ports such as FLRig, FLDigi, JS8Call, or RigCtlD are warnings unless the operator is editing the same radio profile.
- App configuration is a dedicated backup-gated action, not an implied side effect of Save Radio. Review should make it clear that FIO can prepare/update selected software configuration when the user chooses that managed app setup path, and that Save Radio alone will not silently rewrite external app files.
- If Launch Control is shown in the guided flow, it must be labeled `Launch Control` and render the launch-control opt-in/path controls. The guided flow should not present a vague `Launch and Support` section.
...
- `Configure Automatically` visible review text remains compact and operator-readable even when many app paths or profiles are detected.
