...
- Added: Multi-radio Phase A foundation in the settings DB now creates structured tables for device profiles, operating profiles, assignments, coordination policies, and VarAC cluster metadata, and seeds a backward-compatible default device/profile assignment from existing single-radio settings.
- Added: Multi-radio Phase C now supports multiple runtime-active device profiles with one explicit primary compatibility device, a station runtime manager with per-device snapshots, endpoint-keyed JS8 hub safety, and a new `Station Overview` plus multi-active Settings UI.
- Added: Multi-radio Phase D now supports operating-profile CRUD, effective per-device assignment/override workflows in Settings, station-overview policy visibility, and primary-shell enforcement for scheduler, Map, Messages, net-control tabs, background ingest, and Launch Control.
- Added: Multi-radio Phase D schedule target scope now lets HF and Net rows target the station, a device profile, or an operating profile, with primary-runtime filtering in the scheduler, row-level target controls in both schedule editors, and target-aware Net/SOP policy signatures.
