...
def _select_operating_profiles(tab, profile_ids: list[int]) -> None:
    wanted = {int(profile_id) for profile_id in profile_ids}
    found: set[int] = set()
    for row in range(tab.operating_profiles_table.rowCount()):
        widget = tab.operating_profiles_table.cellWidget(row, 0)
        chk = widget.findChild(QCheckBox) if widget is not None else None
        if chk is None:
            continue
        profile_id = int(chk.property("operating_profile_id") or 0)
...
class StubStationRuntimeManager:
    ...
    def primary_runtime_policy(self):
        return dict(FakeStore.active_policy)
...
def _make_window(monkeypatch, tmp_path, active_profile, active_policy):
    ...
    FakeStore.active_policy = dict(active_policy)
...
def test_main_window_applies_primary_operating_profile_policy(monkeypatch, tmp_path):
    ...
    try:
        assert window._suppressed_screen_labels == {
            "Map",
            "Messages",
            "NCS-FLDigi/SSB",
            "NCS-JS8",
